...
export function resumeArgv(cli: AgentCli, id: string): string[] {
  switch (cli) {
    ...
    // The Berth profile assigns Ctrl+C to chat-level interruption, rather than letting Codex
    // interrupt and exit its entire TUI process.
    case 'codex':  return ['--profile', CODEX_BERTH_PROFILE, 'resume', '--no-alt-screen', id]
...
export function freshArgv(cli: AgentCli, o: FreshOpts): string[] {
  ...
  switch (cli) {
    ...
    case 'codex': {
      const pos = positional(o)
      return [
        // The profile owns both the optional SessionStart hook and Berth's Ctrl+C semantics. It
        // must also apply to context-free fresh launches and to resumed sessions.
        '--profile', CODEX_BERTH_PROFILE,
        ...(o.injectFile ? ['--dangerously-bypass-hook-trust'] : []),
...
export function launchFresh(cli: AgentCli, o: FreshOpts, flags: { minimal?: boolean } = {}): IPty {
  ...
  if (cli === 'claude') ensureClaudeTrust(cwd)   // interactive PTY → trust dialog isn't auto-skipped
    ...
    ensureCodexTrust(cwd) // ditto: codex's dir-trust dialog blocks the unattended turn
    ensureCodexBerthHookProfile()
  }
