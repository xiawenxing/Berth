...
// there aren't enough active ones.
//
 ...
 * @param {{staleDays?:number, minVisible?:number}} [opts]
 * @returns {{visible:Array, stale:Array}} both arrays sorted newest-first
 */
export function splitGroupRows(sessions, nowSec, opts = {}) {
  ...
  // Keep all active rows, but never fewer than minVisible (capped at group size).
  const visibleCount = Math.min(Math.max(activeCount, minVisible), sorted.length)
